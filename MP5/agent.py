...
class Agent():
    ...
    def train_policy_net(self, frame):
        ...
        ### CODE ####
        next_state_action: Tensor = self.policy_net(torch.from_numpy(next_states).to(device))

        # Find maximum Q-value of action at next state from policy net
        ### CODE ####
        next_state_values = (next_state_action.max(1)[0] * mask.float()).to(device).detach()
